...
- `npm run test:autotranslate:live` is the explicit network integration check: it exercises the real local detector, Google-backed `translate` engine, multilingual eligibility, team-chat handler, Rust in-game queue, and final `sendTeamMessageAsync` boundary while capturing Discord/Rust outputs instead of publishing them.
